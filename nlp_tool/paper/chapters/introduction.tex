...
The Filipino language is one of the two official languages of the Philippines, the other
one being English~\cite{Ethnologue}. Given that the country has two
official languages that are both widely used in multiple contexts, it is common
...
these standardization efforts have enabled the development of natural language processing
(NLP) software, such as the United States FDA's Phonetic and Orthographic
Computer Analysis (POCA) program, which detects confusable drug names~\cite{FDA_POCA}. However,
these tools are primarily designed for English drug names and may not account for the
...
In this paper, we present a rule-based nativization algorithm for Filipino loan words,
specifically focusing on English drug names. The algorithm applies phonological
transformation rules derived from the Komisyon sa Wikang Filipino (KWF) orthography
guidelines~\cite{KWF_MMP}. We evaluate the method on three datasets and analyze its
performance across different word types.
