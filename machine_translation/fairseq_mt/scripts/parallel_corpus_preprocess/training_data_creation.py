...
def read_tsv(tsv_file: Path):
    """Given a TSV parallel corpus, read the source and target text.

    Args:
        tsv_file (Path): Path to the TSV file

    Returns: 
        Tuple[bool, list[Tuple(str, str)]]: src and tgt parallel corpus pairs
    """
    aligned_pairs = []
...
for tsv_file in tsv_files:
    ...
    print(f"\n{'='*60}\nProcessing: {langA} → {langB}\n{'='*60}")
    
    isValid, aligned_pairs = read_tsv(tsv_file)
...
Next Steps:
 1. ./scripts/fairseq_preprocess
 2. ./scripts/fairseq_train
'''
